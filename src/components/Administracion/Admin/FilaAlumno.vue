<template>
  <tr v-if="!isMobile" class="fila-alumno desktop-view">
    ...
    <template v-else-if="modo === 'personas'">
      ...
      <td class="col-nombre-personas"> <div class="nombre-container">
          <div class="nombre-completo">{{ alumno.nombre }} {{ alumno.apellido }}</div>
        </div>
      </td>
      <td class="col-acciones-personas"> <div class="acciones-container">
          <button class="btn-ingreso" @click="realizarIngreso"> <i class="fas fa-user-plus"></i>
            Realizar Ingreso
          </button>
        </div>
      </td>
  ...
  <div v-else class="alumno-card mobile-view" :class="{ 'expandida': expandida, 'activo': modo === 'alumnos' && alumno.activo }">
    ...
    <template v-else-if="modo === 'personas'">
       <div class="card-header-personas"> <div class="info-basica">
          <div class="dni-mobile">{{ alumno.dni }}</div>
          <div class="nombre-mobile">{{ alumno.nombre }} {{ alumno.apellido }}</div>
        </div>
        <div class="accion-ingreso-mobile"> <button class="btn-ingreso-mobile" @click="realizarIngreso">
...
const props = defineProps({
  alumno: Object, // Mantenemos nombre 'alumno' por consistencia interna
  isMobile: Boolean,
  modo: { // Nueva prop
    type: String,
    default: 'alumnos'
...
// Emitimos los dos eventos
const emit = defineEmits(['verDetalles', 'realizarIngreso'])

const expandida = ref(false)

const toggleExpandida = () => {
  if (props.modo === 'alumnos') { // Solo expandir en modo alumno
...
// Nueva función para emitir el evento de ingreso
...
.fila-alumno.desktop-view td {
  ...
  padding: 0.5rem; /* Reducido para mejor ajuste */
  height: 60px;
  box-sizing: border-box;
}

/* Contenedores para centrado */
.dni-container, .nombre-container, .estado-container, .cuotas-container, .acciones-container {
  display: flex;
...
.alumno-card.mobile-view {
  ...
  width: 100%;
  transition: all 0.3s ease;
  border-left: 4px solid #ccc; /* Gris por defecto para personas */
}
/* Borde activo solo para alumnos */
.alumno-card.mobile-view.activo {
  border-left-color: #47d147;
}
/* Borde inactivo solo para alumnos */
.alumno-card.mobile-view:not(.activo) {
   border-left-color: #ff4d4d; /* Cambiado para inactivos */
}
/* Anulamos borde para personas */
.alumno-card.mobile-view[modo="personas"] {
   border-left: none;
}
...
.info-basica { display: flex; flex-direction: column; flex: 1; }
.dni-mobile { font-weight: 600; font-size: 0.9rem; color: #616161; margin-bottom: 0.3rem; }
.nombre-mobile { font-weight: 700; font-size: 1rem; color: #212121; }
.estado-cuotas-mobile { display: flex; flex-direction: column; align-items: flex-end; margin: 0 0.8rem; gap: 0.3rem; }
.cuotas-mobile { font-weight: 600; font-size: 0.8rem; color: #d81b60; }
.cuotas-mobile.ninguna { color: #388e3c; }
.expand-icon { color: #757575; font-size: 0.9rem; transition: transform 0.3s ease; }
...
.accion-ingreso-mobile {
  margin-left: 0.8rem; /* Espacio a la izquierda del botón */
}
.btn-ingreso-mobile {
...
@media (max-width: 768px) {
  ...
  .btn-ingreso-mobile { padding: 0.5rem 0.8rem; font-size: 0.75rem; }
}
...
@media (max-width: 380px) {
  ...
  .btn-ingreso-mobile { padding: 0.5rem 0.7rem; gap: 0.3rem; }
  .btn-ingreso-mobile i { display: none; } /* Ocultar icono si no cabe */
}
</style>